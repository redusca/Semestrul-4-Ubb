...
public class MainController implements IManageObserver {
    ...
    private void updateTable(Long id, String nume, String prenume, String proba, long puncte) {
        ...
        found = tableView2.getItems().stream().anyMatch(pair -> pair.getKey().getId().equals(id));
        if (found) {
            Pair<Participant,Long> P = tableView2.getItems().stream()
                    .filter(pair -> pair.getKey().getId().equals(id)).toList().get(0);
            tableView2.getItems().remove(P);
            tableView2.getItems().add(new Pair<>(P.getKey(), puncte + P.getValue()));
        }
        else  {
            tableView2.getItems().add(new Pair<>(new Participant(id, nume, prenume, 0), puncte));
        }

        tableView2.getItems().sort((p1, p2) -> p2.getValue().compareTo(p1.getValue()));
    }
