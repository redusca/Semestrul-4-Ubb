...
def Compare_ValidationVSComputed3D(validationInputsFirst, validationInputsSecond, validationOutputs, computedOutputsA,
                                   computedOutputsB
...
def genericTVdata(xx, y, seed=5):
    """
    Functia selecteaza datele de antrenare si validare pentru regresie multivariata
    :param xx: lista de inputs
    :param y:  lista output
    :param seed: seed random
    :type xx: list[list]
    :type y: list
    :return: trainInput si ouput si validare input si output
    :rtype: tuple[list, list, list, list]
    """
    np.random.seed(seed)

    indexes = [i for i in range(len(y))]
    trainSample = np.random.choice(indexes, int(0.8 * len(y)), replace=False)
    validationSample = [i for i in indexes if not i in trainSample]

    trainList = [xx[i] for i in trainSample]
    trainOutput = [y[i] for i in trainSample]

    validationList = [xx[i] for i in validationSample]
    validationOutput = [y[i] for i in validationSample]

    return trainList, trainOutput, validationList, validationOutput


def kFodsCrossTrainData(xx, y, k, seed=5):
    np.random.seed(seed)
    ...
    j = 0
    while j <= len(y):
        fieldIndexs.append(j)
        j += len(y) // k

    Rezults = []
    for i in range(len(fieldIndexs) - 1):
        VI = [xx[indexes[j]] for j in range(fieldIndexs[i], fieldIndexs[i + 1])]
        VO = [y[indexes[j]] for j in range(fieldIndexs[i], fieldIndexs[i + 1])]
        TI = [xx[indexes[j]] for j in range(len(y)) if j < fieldIndexs[i] or j >= fieldIndexs[i + 1]]
        TO = [y[indexes[j]] for j in range(len(y)) if j < fieldIndexs[i] or j >= fieldIndexs[i + 1]]
        Rezults.append((TI, TO, VI, VO))
